...
\begin{definition}[Chosen cycles, boundaries, and pages]
  ...
  $B_\infty=\bigcup_{r\ge1} B_r$, together with the quotient
  $E_\infty=Z_\infty/B_\infty$, when these operations exist in the chosen
  algebraic setting.
\end{definition}
...
\begin{definition}[Cycle, boundary, and permanent cycle]
  ...
  the displayed $d_r$ means that its coset has a representative in $Z_r$.
  This is an existence statement because a different $E_2$ representative
  may differ by $B_{r-1}$.  A page class survives to $E_\infty$ when some
  compatible representative lies in $Z_\infty$; its limiting class is the
  corresponding coset in $Z_\infty/B_\infty$, and is zero precisely when the
  permanent representative lies in $B_\infty$.
\end{definition}
...
\begin{definition}[Strong convergence with detection]
  ...
  \lean{KIP126.Classical.Adams.StrongClassicalAdamsConvergence}
  \notready
  ...
  $F^sA_n$, a stable-page bound $R(s,n)$, and isomorphisms
  \[
    E_r^{s,s+n}\cong F^sA_n/F^{s+1}A_n
    \qquad(r\ge R(s,n))
  \]
  compatible with every page-passage isomorphism.  It also records a
  degreewise regularity hypothesis strong enough to rule out the relevant
  Boardman obstruction; completeness and separatedness alone are not used as
  a substitute for this hypothesis.  The implemented classical structure
  contains the stable-page coherence, completion, exhaustiveness, and
  separatedness data.  A generic regularity interface and the elementwise
  detection relation remain pending.

  Under these data, $x\in E_\infty^{s,s+n}$ detects $\alpha\in A_n$ exactly
  when $\alpha\in F^sA_n$ and its image in
  $F^sA_n/F^{s+1}A_n$ is the image of $x$ under the limiting comparison.
  Detection determines the lift only modulo $F^{s+1}A_n$; it is a relation,
  not a chosen function from page classes to abutment elements.
\end{definition}
...
\begin{proposition}[Categorical and elementwise presentations agree]
  ...
  \notready
  Let a spectral sequence in graded $\mathbb F_2$-vector spaces be constructed
  from a specified exact couple or filtered complex, and let its elementwise
  presentation be the one induced by that same construction.  After the
  stored page reindexing, there are natural isomorphisms
  $E_r\cong Z_{r-1}/B_{r-1}$ for every $r\ge2$.  They commute with page
  passage and identify the Mathlib page differential with the map induced by
  the representative homomorphism
  $\widetilde d_r:Z_{r-1}\to E_2/B_{r-1}$.  The proposition makes no claim
  for an arbitrary independently chosen family of subgroups named $Z$ and
  $B$.
\end{proposition}
...
\begin{definition}[Filtered-complex to spectral-object adapter]
  ...
  separate obligations.  The current implementation reaches functorial
  morphisms of triangulated and abelian spectral objects.  It does not yet
  package the induced Mathlib spectral-sequence morphism, preservation of
  boundary/comparison witnesses, separatedness, or the elementwise $Z/B$
  presentation; these are precisely the remaining obligations.
\end{definition}
...
\begin{theorem}[Filtered complex produces a spectral sequence]
  ...
  have a filtration-preserving differential.  Assume the full adapter input
  of Definition~\ref{def:filtered-complex-spectral-object-adapter}, and assume
  an initial-page comparison identifying the recipe's initial displayed page
  with the associated-graded complex.  Then the adapter produces the stated
  Mathlib spectral sequence.  If, in addition, coherent strong-convergence
  data identify the endpoint abutment with filtered homology, its limiting
  page is the associated graded of filtered homology.  If the induced
  categorical--elementwise comparison is supplied, the stored display
  reindexing gives $E_r\cong Z_{r-1}/B_{r-1}$.

  For functoriality, a filtered chain map must be accompanied by compatible
  maps of the chosen endpoint extensions, boundary witnesses, recipes, and
  convergence comparisons.  Under those hypotheses it induces a Mathlib
  spectral-sequence morphism whose initial component is the associated-graded
  chain map; identities and composition are preserved.  No claim is made that
  an arbitrary filtered chain map automatically preserves independently
  chosen endpoint or convergence data.
\end{theorem}
...
\begin{proof}
  ...
  spectral sequence.  Identify its categorical cycles and opcycles with the
  filtration kernels and cokernels and hence, with the recorded predecessor
  offset, with $Z_{r-1}$ and $B_{r-1}$ on the displayed $E_r$ page.  Finally use
  ...
  homology.  For a filtered chain map with the compatibility data above, apply
  functoriality of the cochain diagram, mapping-cone construction, and
  homological-image bridge, then construct the spectral-sequence morphism and
  prove that its initial component is the associated-graded chain map.
\end{proof}
